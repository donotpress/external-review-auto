...
| `claude-no-output` | Legacy first-byte-death trailer (pre-split; nothing emits it anymore) | Dead-transport fallback (REST), even in usable rounds | Stderr trailer → `Convert-EraAdapterResultError` at collection |
| `claude-first-byte-timeout` | First-byte bound elapsed with zero stdout bytes (in text mode the bound is a total cap, so usually a healthy slow review) | Fallback when round void; NO extra REST re-dispatch in usable rounds | Stderr trailer → `Convert-EraAdapterResultError` at collection |
