...
14. **Bash tool timeout kills `era` early (opencode, measured 2026-09-02):** the `bash` tool defaults to `timeout: 120000ms`. `era.ps1` scales its own `TimeoutSec` to the bundle (`600s` base → `1800s` for a `116k`-token 3-4-seat panel + `300s` straggler grace for the last seat). A `120s` outer timeout `SIGTERM`s the `pwsh` dispatch while `deepseek-flash`/`muse-spark` are still running, leaving `round-N-*.pid` but no `.md` — the exact failure that lost `deepseek-flash` on `bulk-refresh-vpn-headless` round 1 (`451s` still in grace at `120s` kill). **Fix:** when dispatching via `bash`, always pass `timeout: 600000` for a single reviewer or `timeout: 1800000` for a panel. Example:
   ```bash
   pwsh -Command "Set-Location 'C:\...\repo'; & '...\runtimes\era.ps1' -Reviewer 'gemini,deepseek-flash,muse-spark' -TopicSlug 'my-topic' -IncludeFiles 'a.py,b.py' -Force"  # bash timeout: 1800000
   ```
   `SKILL.md` step 5 and `runtimes/opencode.md` now document this. If you already hit it, the orphaned `round-N-claim.json` will skip that round number — delete it before re-dispatching (`Remove-Item .external-reviews/<topic>/round-*-claim.json`) and re-run with the longer timeout. Exit 2 after a kill is not "no findings" — it means money was spent and nothing was reviewed.
