...
14. **A driving tool's timeout fires before `era` finishes (measured 2026-09-02):** era scales its own `TimeoutSec` to the bundle at 20 ms/token — 600 s floor, **1800 s ceiling, reached at 90,000 tokens** — plus 300 s of straggler grace for the last seat. Every driving tool waits less than that. **The fix is to dispatch in the background, not to raise the timeout**, because the timeout cannot be raised far enough: on Claude Code the Bash foreground wait is capped at **600 s**, and `timeout: 1800000` is accepted and silently clamped to it.
