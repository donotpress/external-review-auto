...
**Fix:** the first-token watchdog (Phase 1) now kills the process at the `ERA_OPENCODE_FIRST_TOKEN_SEC` deadline (default 120s, min 10s) if zero output has ever been captured. This catches the popup case at ~120–130s instead of waiting 10–30 minutes. If Phase 1 misses (e.g. the popup produces at least one byte), Phase 2 catches it at the stall threshold — since 2026-09-04 a measured `prefill + generation` figure that floors at **824s** and is then clamped to `budget − 30s`, so on a typical seat budget Phase 2 fires 30s before the timeout rather than minutes before it. That is deliberate: silence does not distinguish a working seat from a wedged one (the two distributions overlap almost completely), and the old 120–600s window killed 8.07% of productive seat turns at its `default` tier. See `docs/assessments/2026-09-04-stall-threshold-measured.md`. Set `ERA_OPENCODE_FIRST_TOKEN_SEC` to a lower value (e.g. `60`) to fail faster at the cost of false-positive risk on very slow models.
