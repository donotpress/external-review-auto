...
| `ERA_PREFLIGHT_ONLY` | (unset) | Set to `1` to build the bundle, run **every** gate (broad scope, dirty tree, per-seat delivery ceilings, cost estimate, blind-seat strip) and then stop **before dispatching any reviewer** — exit 0, nothing sent, nothing spent. Useful on its own to answer "what would this round cost and would every seat fit?", and required by the tests that fork a real `era.ps1` to exercise pre-dispatch behaviour. Those forks passed no `-IncludeFiles` and no `-Reviewer`, so era did the correct thing and dispatched the full default panel: measured 2026-09-04, a suite run spawned a real `opencode run`, held `Global\era-opencode-run-mutex`, and blocked a concurrent live round for 476 s. |
