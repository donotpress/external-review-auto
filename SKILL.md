...
> model out of the default panel. On 2026-09-05 the preset was **deleted**: `retired`
> turned out to be read by nothing at runtime, only by two test files, so an explicit
> `-Reviewer ox-alpha` still dispatched, spawned an opencode process, and failed at the
> vendor rather than at era. The flag keeps a withdrawn model out of the *default panel*;
> it does not make one fail loudly on request, and the note claiming otherwise was wrong.
...
5. **Dispatch** — `pwsh <skill-root>/runtimes/era.ps1 <flags> -Force`
   - **A long panel needs a background dispatch, not a bigger timeout (measured 2026-09-02).** era scales its own `TimeoutSec` to the bundle at 20 ms/token — 600 s floor, **1800 s ceiling, reached at 90,000 tokens** — plus 300 s of straggler grace for the last seat. Any real panel is therefore budgeted longer than a driving tool will wait in the foreground. On Claude Code the Bash foreground wait is **capped at 600 s**: `timeout: 1800000` is accepted and silently clamped, so no foreground setting covers a full panel. **Dispatch with `run_in_background: true` (or `setsid`) and poll the log.** Not `nohup ... &` inside a foreground call -- `nohup` blocks SIGHUP but stays in the call's process group, which is reaped wholesale when the call ends; that is exactly how `direction-paths-2026-09-04` round 1 was killed ~60 s in. Measured: an over-running `pwsh` child is *moved to the background* and runs to completion (200 s child under the 120 s default, 640 s child under the 600 s cap — both `RC=0`), so a timeout message is **not** evidence the round died. Re-dispatching on that assumption spends the round twice. See `references/troubleshooting.md` item 14.
...
| `ERA_AGY_FALLBACK` | (auto) | **Legacy spelling** of `ERA_FALLBACK_PRESET` below — kept working forever. v1.10: when an **agy** reviewer fails to capture even after its retry, era auto-falls-back to a non-agy reviewer so the round still yields a review. Set to a preset (e.g. `gemini-api`) to pin the fallback, or `off`/`0` to disable. Triggers only on an actual agy failure — healthy runs are unaffected. |
| `ERA_FALLBACK_PRESET` | (unset) | Documented name for the fallback override (`ERA_AGY_FALLBACK` still works; this wins when both are set). Same values: a preset to pin, or `off`/`0` to disable all fallback re-dispatch. Applies to any recoverable seat failure, not just agy ones. |
| `ERA_IGNORE_QUOTA_FLAG` | (unset) | Set to `1` to ignore the agy quota-exhausted flag and dispatch gemini-family seats anyway. The flag itself lives at `$env:TEMP/era-agy-quota.json` (`{"exhausted": true, "refresh_utc": "..."}`), written by hand or by a `/usage`-screen probe; while a fresh exhausted flag exists, agy gemini seats fail instantly (`agy-quota-exhausted`, zero stall burned) instead of dispatching into an empty pool. Missing/malformed/expired flags behave as if absent. |
