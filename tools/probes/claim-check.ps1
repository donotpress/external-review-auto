<#
.SYNOPSIS
    claim-check: per-seat citation grounding for delivered reviews.
.DESCRIPTION
    For each response file: extract `path:line` / `path:start-end`
    citations (backticked or bare, the two dominant forms measured across
    archived rounds), check each against the bundle frame (file spans parsed
    from the sibling round-N-bundle.xml) and the disk frame (paths joined
    on -RepoRoot when given), and write `<response>.check.json` beside it:
    totals plus the ungrounded list. Advisory instrument: receipts always
    written when the response is readable; a missing explicit response
    throws; an absent corpus prints 'nothing to measure' and returns.

    With no -ResponsePath, scans every per-preset round response under this
    skill's own .external-reviews (resolved from $PSScriptRoot, never
    hardcoded) -- the same convention as citation-grounding.ps1.
...
.EXAMPLE
    pwsh tools/probes/claim-check.ps1
#>
[CmdletBinding()]
param(
    [string]$ResponsePath,
...
$explicitBundle = $PSBoundParameters.ContainsKey('BundlePath')
$targets = @()
if ($ResponsePath) {
    if (-not (Test-Path -LiteralPath $ResponsePath)) {
        throw "[claim-check] no response at '$ResponsePath' -- nothing to check."
    }
    $targets = @($ResponsePath)
} else {
    $reviewsRoot = Join-Path (Split-Path -Parent (Split-Path -Parent $PSScriptRoot)) '.external-reviews'
    if (Test-Path -LiteralPath $reviewsRoot) {
        $targets = @(Get-ChildItem -LiteralPath $reviewsRoot -Recurse -Filter 'round-*-response.md' -File -ErrorAction SilentlyContinue |
            Where-Object { $_.Name -match '^round-(\d+)-(.+)response\.md$' } |
            ForEach-Object { $_.FullName })
    }
    if (@($targets).Count -eq 0) {
        Write-Host "[claim-check] nothing to measure."
        return
    }
}

foreach ($resp in $targets) {
    $bPath = $BundlePath
    if (-not $explicitBundle) {
        $bPath = $null
        $dir = Split-Path -Parent $resp
        $base = [System.IO.Path]::GetFileName($resp)
        if ($base -match '^round-(\d+)-.*response\.md$') {
            $cand = Join-Path $dir ("round-$($matches[1])-bundle.xml")
            if (Test-Path -LiteralPath $cand) { $bPath = $cand }
        }
    }

    # Bundle frame: path -> line count, same shape as citation-grounding.
    $bundleSpans = @{}
    if ($bPath -and (Test-Path -LiteralPath $bPath)) {
        try {
            $raw = Get-Content -Raw -LiteralPath $bPath -ErrorAction Stop
    ...
    $text = Get-Content -Raw -LiteralPath $resp -ErrorAction Stop
    ...
    $bn = [System.IO.Path]::GetFileName($resp)
